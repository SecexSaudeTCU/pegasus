...
"""
Cria o schema do banco de dados do CNES_IN (Incentivos) para atender ao formato do SGBD postgreSQL.
Na verdade o schema abaixo deve atender ao formato dos SGBD Oracle e SQL Server com no máximo pequenas modificações.

"""

# Incorpora o schema do banco de dados do CNES_IN definido nas classes abaixo
Base = declarative_base()

# Tabela dos Incentivos (IN) (tabela principal do "sub" banco de dados CNES_IN)
class INBR(Base):
    __tablename__ = 'inbr'
    __table_args__ = {'schema': 'cnes_in'}
    ...
    CNES_ID = Column(String(7), ForeignKey('cnes_in.cnes.ID'))                  # Primary (should be!) and Foreign key
    cnes = relationship('CNES')
    UF_IN = Column(String(2))                                                   # Atributo
    ANO_IN = Column(Integer)                                                    # Atributo
    MES_IN = Column(String(2))                                                  # Atributo
    CODUFMUN_ID = Column(String(6), ForeignKey('cnes_in.codufmun.ID'))          # Foreign key
    codufmun = relationship('CODUFMUN')
    TPGESTAO_ID = Column(String(2), ForeignKey('cnes_in.tpgestao.ID'))          # Foreign key
    tpgestao = relationship('TPGESTAO')
    PFPJ_ID = Column(String(2), ForeignKey('cnes_in.pfpj.ID'))                  # Foreign key
    pfpj = relationship('PFPJ')
    CPF_CNPJ = Column(String(14))                                               # Atributo
    NIVDEP_ID = Column(String(2), ForeignKey('cnes_in.nivdep.ID'))              # Foreign key
    nivdep = relationship('NIVDEP')
    CNPJ_MAN = Column(String(14))                                               # Atributo
    ESFERAA_ID = Column(String(2), ForeignKey('cnes_in.esferaa.ID'))            # Foreign key
    esferaa = relationship('ESFERAA')
    RETENCAO_ID = Column(String(2), ForeignKey('cnes_in.retencao.ID'))          # Foreign key
    retencao = relationship('RETENCAO')
    ATIVIDAD_ID = Column(String(2), ForeignKey('cnes_in.atividad.ID'))          # Foreign key
    atividad = relationship('ATIVIDAD')
    NATUREZA_ID = Column(String(2), ForeignKey('cnes_in.natureza.ID'))          # Foreign key
    natureza = relationship('NATUREZA')
    CLIENTEL_ID = Column(String(2), ForeignKey('cnes_in.clientel.ID'))          # Foreign key
    clientel = relationship('CLIENTEL')
    TPUNID_ID = Column(String(2), ForeignKey('cnes_in.tpunid.ID'))              # Foreign key
    tpunid = relationship('TPUNID')
    TURNOAT_ID = Column(String(2), ForeignKey('cnes_in.turnoat.ID'))            # Foreign key
    turnoat = relationship('TURNOAT')
    NIVHIER_ID = Column(String(2), ForeignKey('cnes_in.nivhier.ID'))            # Foreign key
    ...
    TPPREST_ID = Column(String(2), ForeignKey('cnes_in.tpprest.ID'))            # Foreign key
    ...
    NATJUR_ID = Column(String(4), ForeignKey('cnes_in.natjur.ID'))              # Foreign key
...
class CODUFMUN(Base):
    ...
    UFCOD_ID = Column(String(2), ForeignKey('cnes_in.ufcod.ID'))                # Foreign key
