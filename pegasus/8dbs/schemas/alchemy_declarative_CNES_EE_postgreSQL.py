...
"""
Cria o schema do banco de dados do CNES_EE (Estabelecimentos de Ensino) para atender ao formato do SGBD postgreSQL.
Na verdade o schema abaixo deve atender ao formato dos SGBD Oracle e SQL Server com no máximo pequenas modificações.

"""

# Incorpora o schema do banco de dados do CNES_EE definido nas classes abaixo
Base = declarative_base()

# Tabela dos Estabelecimentos de Ensino (EE) (tabela principal do "sub" banco de dados CNES_EE)
class EEBR(Base):
    __tablename__ = 'eebr'
    __table_args__ = {'schema': 'cnes_ee'}
    ...
    CNES_ID = Column(String(7), ForeignKey('cnes_ee.cnes.ID'))                  # Primary (should be!) and Foreign key
    cnes = relationship('CNES')
    UF_EE = Column(String(2))                                                   # Atributo
    ANO_EE = Column(Integer)                                                    # Atributo
    MES_EE = Column(String(2))                                                  # Atributo
    CODUFMUN_ID = Column(String(6), ForeignKey('cnes_ee.codufmun.ID'))          # Foreign key
    codufmun = relationship('CODUFMUN')
    TPGESTAO_ID = Column(String(2), ForeignKey('cnes_ee.tpgestao.ID'))          # Foreign key
    tpgestao = relationship('TPGESTAO')
    PFPJ_ID = Column(String(2), ForeignKey('cnes_ee.pfpj.ID'))                  # Foreign key
    pfpj = relationship('PFPJ')
    CPF_CNPJ = Column(String(14))                                               # Atributo
    NIVDEP_ID = Column(String(2), ForeignKey('cnes_ee.nivdep.ID'))              # Foreign key
    nivdep = relationship('NIVDEP')
    CNPJ_MAN = Column(String(14))                                               # Atributo
    ESFERAA_ID = Column(String(2), ForeignKey('cnes_ee.esferaa.ID'))            # Foreign key
    esferaa = relationship('ESFERAA')
    RETENCAO_ID = Column(String(2), ForeignKey('cnes_ee.retencao.ID'))          # Foreign key
    retencao = relationship('RETENCAO')
    ATIVIDAD_ID = Column(String(2), ForeignKey('cnes_ee.atividad.ID'))          # Foreign key
    atividad = relationship('ATIVIDAD')
    NATUREZA_ID = Column(String(2), ForeignKey('cnes_ee.natureza.ID'))          # Foreign key
    natureza = relationship('NATUREZA')
    CLIENTEL_ID = Column(String(2), ForeignKey('cnes_ee.clientel.ID'))          # Foreign key
    clientel = relationship('CLIENTEL')
    TPUNID_ID = Column(String(2), ForeignKey('cnes_ee.tpunid.ID'))              # Foreign key
    tpunid = relationship('TPUNID')
    TURNOAT_ID = Column(String(2), ForeignKey('cnes_ee.turnoat.ID'))            # Foreign key
    turnoat = relationship('TURNOAT')
    NIVHIER_ID = Column(String(2), ForeignKey('cnes_ee.nivhier.ID'))            # Foreign key
    ...
    TPPREST_ID = Column(String(2), ForeignKey('cnes_ee.tpprest.ID'))            # Foreign key
    tpprest = relationship('TPPREST')
    SGRUPHAB_ID = Column(String(4), ForeignKey('cnes_ee.sgruphab.ID'))          # Foreign key
    ...
    NATJUR_ID = Column(String(4), ForeignKey('cnes_ee.natjur.ID'))              # Foreign key
...
class CODUFMUN(Base):
    ...
    UFCOD_ID = Column(String(2), ForeignKey('cnes_ee.ufcod.ID'))                # Foreign key
