...
"""
Cria o schema do banco de dados do CNES_HB (Habilitações) para atender ao formato do SGBD postgreSQL.
Na verdade o schema abaixo deve atender ao formato dos SGBD Oracle e SQL Server com no máximo pequenas modificações.

"""

# Incorpora o schema do banco de dados do CNES_HB definido nas classes abaixo
Base = declarative_base()

# Tabela das Habilitações (HB) (tabela principal do "sub" banco de dados CNES_HB)
class HBBR(Base):
    __tablename__ = 'hbbr'
    __table_args__ = {'schema': 'cnes_hb'}
    ...
    CNES_ID = Column(String(7), ForeignKey('cnes_hb.cnes.ID'))                  # Primary (should be!) and Foreign key
    cnes = relationship('CNES')
    UF_HB = Column(String(2))                                                   # Atributo
    ANO_HB = Column(Integer)                                                    # Atributo
    MES_HB = Column(String(2))                                                  # Atributo
    CODUFMUN_ID = Column(String(6), ForeignKey('cnes_hb.codufmun.ID'))          # Foreign key
    codufmun = relationship('CODUFMUN')
    TERCEIRO = Column(Numeric)                                                  # Atributo
    SGRUPHAB_ID = Column(String(4), ForeignKey('cnes_hb.sgruphab.ID'))          # Foreign key
...
class CODUFMUN(Base):
    ...
    UFCOD_ID = Column(String(2), ForeignKey('cnes_hb.ufcod.ID'))                # Foreign key
