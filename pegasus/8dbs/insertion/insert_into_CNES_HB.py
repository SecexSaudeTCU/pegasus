...
from .data_wrangling.prepare_CNES_HB import *
...
def insert_most_CNES_HB_tables_pandas(path, device, child_db):
...
def insert_main_table_e_file_info_pandas(file_name, directory, date_ftp, device, child_db, parent_db):
    ...
    # Tratamento de dados principais do CNES_HB
    state = file_name[2:4]
    year = file_name[4:6]
    month = file_name[6:8]
    main_table = 'hbbr'
    ...
    print(f'\nIniciando a lida com o arquivo HB{state}{year}{month}...')
    # Chama a função "get_HBXXaamm_treated" do módulo "prepare_CNES_HB" do package "data_wrangling"
    df = get_HBXXaamm_treated(state, year, month)
    # Inserção das colunas UF_HB, ANO_HB e MES_HB no objeto pandas DataFrame "df"
    df.insert(1, 'UF_HB', [state]*df.shape[0])
    df.insert(2, 'ANO_HB', [int('20' + year)]*df.shape[0])
    df.insert(3, 'MES_HB', [month]*df.shape[0])
    ...
    print(f'Terminou de inserir os dados do arquivo HB{state}{year}{month} na tabela {main_table} do banco de dados {child_db}.')
    ...
    print(f'Terminou de inserir os metadados do arquivo HB{state}{year}{month} na tabela arquivos do banco de dados {child_db}.')
    end = time.time()
