...
"""
Script de tratamento de dados do CNES_HB (Habilitações) para atender ao framework do SGBD PostgreSQL.
Válido para os arquivos de dados HBXXaamm (HB = Habilitações; XX = Estado; aa = Ano; mm = Mês)...
...
def get_HBXXaamm_treated(state, year, month):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e o salva no formato "parquet"
    dataframe = download_CNESXXaamm('HB', state, year, month)
    print('O número de linhas do arquivo HB{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))

    # Colunas definidas como necessárias no objeto pandas DataFrame que incrementará a tabela HBBR da base de dados
    ...
    print('Terminou de tratar o arquivo HB{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
# Função para adequar e formatar as colunas e valores da Tabela HABILITA (arquivo HABILITA.dbf)
def get_HABILITA_treated():
...
if __name__ == '__main__':
    ...
    df = get_HBXXaamm_treated('ES', '12', '10')
    print(df)
