...
"""
Script de tratamento de dados do CNES_EE (Estabelecimentos de Ensino) para atender ao framework do SGBD PostgreSQL.
Válido para os arquivos de dados EEXXaamm (EF = Estabelecimentos de Ensino; XX = Estado; aa = Ano; mm = Mês)...
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados EEXXaamm do CNES e adequar e formatar suas colunas e valores
def get_EEXXaamm_treated(state, year, month):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e ainda o salva no formato "parquet"
    dataframe = download_CNESXXaamm('EE' , state, year, month)
    print('O número de linhas do arquivo EE{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))

    # Colunas definidas como necessárias no objeto pandas DataFrame que incrementará a tabela EEBR da base de dados
    ...
    print('Terminou de tratar o arquivo EE{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
def get_CADGERBR_treated(path):
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela EEBR
...
# Função para adequar e formatar as colunas e valores da TCC ESTABENS (arquivo ESTABENS.cnv)
def get_ESTABENS_treated():
