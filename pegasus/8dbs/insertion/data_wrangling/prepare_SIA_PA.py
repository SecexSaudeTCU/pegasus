...
Válido para os arquivos de dados PAXXaamm (PA = Procedimentos Ambulatoriais; XX = Estado; aa = Ano; mm = Mês)...
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados PAXXaamm do SIA e adequar e formatar suas colunas e valores
def get_PAXXaamm_treated(state, year, month):
    ...
    print('O número de linhas do arquivo PA{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))

    # Colunas definidas como necessárias no objeto pandas DataFrame que incrementará a tabela PABR da base de dados
    lista_columns = np.array(['PA_CODUNI', 'PA_GESTAO', 'PA_CONDIC', 'PA_UFMUN', 'PA_REGCT', 'PA_INCOUT', 'PA_INCURG',
                              'PA_TPUPS', 'PA_TIPPRE', 'PA_MN_IND', 'PA_CNPJCPF', 'PA_CNPJMNT', 'PA_CNPJ_CC', 'PA_MVM',
    ...
    df['PA_CATEND'].replace(['0', '7', '10', '12', '20', '53', '54', '57', '0-', '0E', '0U'], '', inplace=True)
    ...
    for col in np.array(['PA_CODUNI', 'PA_GESTAO', 'PA_CONDIC', 'PA_UFMUN', 'PA_REGCT', 'PA_TPUPS', 'PA_TIPPRE',
                         'PA_PROC_ID', 'PA_TPFIN', 'PA_NIVCPL', 'PA_DOCORIG', 'PA_CBOCOD', 'PA_MOTSAI',
                         'PA_CIDPRI', 'PA_CIDSEC', 'PA_CIDCAS', 'PA_CATEND', 'PA_FLIDADE', 'PA_SEXO', 'PA_RACACOR',
                         'PA_MUNPCN', 'PA_INDICA', 'PA_CODOCO', 'PA_ETNIA', 'PA_SRC_C', 'PA_INE', 'PA_NAT_JUR']):
        df[col].replace('', 'NA', inplace=True)

    # Substitui uma string vazia por None nas colunas de atributos especificadas
    for col in np.array(['PA_MN_IND', 'PA_CNPJCPF', 'PA_CNPJMNT', 'PA_CNPJ_CC', 'PA_AUTORIZ', 'PA_CNSMED']):
    ...
    df.rename(index=str, columns={'PA_CODUNI': 'PACODUNI_ID', 'PA_GESTAO': 'PAGESTAO_ID', 'PA_CONDIC': 'PACONDIC_ID',
                                  'PA_UFMUN': 'PAUFMUN_ID', 'PA_REGCT': 'PAREGCT_ID', 'PA_TPUPS': 'PATPUPS_ID',
                                  'PA_TIPPRE': 'PATIPPRE_ID', 'PA_PROC_ID': 'PAPROC_ID', 'PA_TPFIN': 'PATPFIN_ID',
                                  'PA_NIVCPL': 'PANIVCPL_ID', 'PA_DOCORIG': 'PADOCORIG_ID', 'PA_CBOCOD': 'PACBOCOD_ID',
                                  'PA_MOTSAI': 'PAMOTSAI_ID', 'PA_CIDPRI': 'PACIDPRI_ID', 'PA_CIDSEC': 'PACIDSEC_ID',
                                  'PA_CIDCAS': 'PACIDCAS_ID', 'PA_CATEND': 'PACATEND_ID', 'PA_FLIDADE': 'PAFLIDADE_ID',
                                  'PA_SEXO': 'PASEXO_ID', 'PA_RACACOR': 'PARACACOR_ID', 'PA_MUNPCN': 'PAMUNPCN_ID',
                                  'PA_INDICA': 'PAINDICA_ID', 'PA_CODOCO': 'PACODOCO_ID', 'PA_ETNIA': 'PAETNIA_ID',
                                  'PA_SRC_C': 'PASRCC_ID', 'PA_INE': 'PAINE_ID', 'PA_NAT_JUR': 'PANATJUR_ID'}, inplace=True)

    print('Terminou de tratar o arquivo PA{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
def get_CADGERBR_treated(path):
    ...
    # Upload do arquivo "xlsx" que contém os CNES presentes nos arquivos PAXXaamm (dos anos de 2008 a 2019) e não presentes...
    # nas 27 Tabelas CADGERXX. Ou seja, isso parece ser uma falha dos dados do Datasus
    dataframe = pd.read_excel(path + 'PA_CODUNI_OUT_CADGER_XX_ANOS_2008_2019' + '.xlsx')
...
def get_CADMUN_treated():
    ...
    df.reset_index(drop=True, inplace=True)
    # Inserção de linha atípica mas mencionada no Dicionário de Dados do SIA
    df.loc[df.shape[0]] = ['UF0000', 'GESTAO ESTADUAL', '?', '?', '?', '?', '?', '?', 'NA', '?', '?', '?', np.nan, np.nan, np.nan, np.nan, '?', '?', '?']
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela PABR
...
def get_S_CID_treated():
    ...
    ## Drop a linha inteira em que a coluna "DIAGNOSTICO" tem os valores especificados por não representarem valor útil
...
def get_CODOCO_treated():
    ...
    df.loc[df.ID.str.startswith('1'), 'OCORRENCIA'] = df.loc[df.ID.str.startswith('1'), 'OCORRENCIA'].replace(to_replace='^\.\.',
                                                                                                              value= 'PRODUÇÃO TOT. APROVADA - ',
                                                                                                              regex=True)
    df.loc[df.ID == '2L', 'OCORRENCIA'] = df.loc[df.ID == '2L', 'OCORRENCIA'].replace(to_replace='^\.\.',
                                                                                      value= 'PRODUÇÃO PARC. APROVADA - ',
                                                                                      regex=True)
    df.loc[df.ID.str.startswith('3'), 'OCORRENCIA'] = df.loc[df.ID.str.startswith('3'), 'OCORRENCIA'].replace(to_replace='^\.\.',
                                                                                                              value= 'PRODUÇÃO PARC. APROVADA - ',
                                                                                                              regex=True)
    df.loc[df.ID.str.startswith('4'), 'OCORRENCIA'] = df.loc[df.ID.str.startswith('4'), 'OCORRENCIA'].replace(to_replace='^\.\.',
                                                                                                              value= 'PRODUÇÃO NAO APROVADA - ',
                                                                                                              regex=True)
    df.loc[df.ID == '5O', 'OCORRENCIA'] = df.loc[df.ID == '5O', 'OCORRENCIA'].replace(to_replace='^\.\.',
                                                                                      value= 'PRODUÇÃO NAO APROVADA - ',
                                                                                      regex=True)
...
def get_TABUF_treated():
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" das tabelas de municípios
    df.loc[df.shape[0]] = ['NA', 'NOT AVAILABLE', '?']
    return df
