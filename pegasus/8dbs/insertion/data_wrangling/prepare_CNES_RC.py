...
"""
Script de tratamento de dados do CNES_RC (Regras Contratuais) para atender ao framework do SGBD PostgreSQL.
Válido para os arquivos de dados RCXXaamm (RC = Regras Contratuais; XX = Estado; aa = Ano; mm = Mês)...
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados RCXXaamm do CNES e adequar e formatar suas colunas e valores
def get_RCXXaamm_treated(state, year, month):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e o salva no formato "parquet"
    dataframe = download_CNESXXaamm('RC', state, year, month)
    print('O número de linhas do arquivo RC{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))
    ...
    print('Terminou de tratar o arquivo RC{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
# Função para adequar e formatar as colunas e valores da Tabela REGRAS (arquivo REGRAS.dbf)
def get_REGRAS_treated():
