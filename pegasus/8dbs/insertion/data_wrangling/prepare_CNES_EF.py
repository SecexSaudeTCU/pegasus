...
"""
Script de tratamento de dados do CNES_EF (Estabelecimentos Filantrópicos) para atender ao framework do SGBD PostgreSQL.
Válido para os arquivos de dados EFXXaamm (EF = Estabelecimentos Filantrópicos; XX = Estado; aa = Ano; mm = Mês)...
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados EFXXaamm do CNES e adequar e formatar suas colunas e valores
def get_EFXXaamm_treated(state, year, month):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e ainda o salva no formato "parquet"
    dataframe = download_CNESXXaamm('EF' , state, year, month)
    print('O número de linhas do arquivo EF{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))

    # Colunas definidas como necessárias no objeto pandas DataFrame que incrementará a tabela EFBR da base de dados
    ...
    print('Terminou de tratar o arquivo EF{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
def get_CADGERBR_treated(path):
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela EFBR
...
# Função para adequar e formatar as colunas e valores da TCC ESTABFIL (arquivo ESTABFIL.cnv)
def get_ESTABFIL_treated():
