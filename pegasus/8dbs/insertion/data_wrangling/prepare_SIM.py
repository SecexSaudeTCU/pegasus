...
(originalmente em formato "dbc") a partir do ano de 1997.
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados DOXXaaaa do SIM e adequar e formatar suas colunas e valores
def get_DOXXaaaa_treated(state, year):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e o salva no formato "parquet"
    dataframe = download_DOXXaaaa(state, year)
    print('O número de linhas do arquivo DO{}{} é {}.'.format(state, year, dataframe.shape[0]))
    ...
    lista_columns = np.array(['NUMERODO', 'CODINST', 'TIPOBITO', 'DTOBITO', 'HORAOBITO', 'NUMSUS', 'NATURAL', 'CODMUNNATU', 'DTNASC',
                              'IDADE', 'SEXO', 'RACACOR', 'ESTCIV', 'ESC', 'ESC2010', 'OCUP', 'CODMUNRES', 'LOCOCOR', 'CODESTAB',
                              'CODMUNOCOR', 'TPMORTEOCO', 'ASSISTMED', 'EXAME', 'CIRURGIA', 'NECROPSIA', 'LINHAA', 'LINHAB', 'LINHAC',
                              'LINHAD', 'LINHAII', 'CAUSABAS', 'CRM', 'DTATESTADO', 'CIRCOBITO', 'ACIDTRAB', 'FONTE', 'TPPOS', 'DTINVESTIG',
                              'CAUSABAS_O', 'DTCADASTRO', 'ATESTANTE', 'FONTEINV', 'DTRECEBIM', 'ATESTADO', 'ESCMAEAGR1', 'ESCFALAGR1',
                              'STDOEPIDEM', 'STDONOVA', 'DIFDATA', 'DTCADINV', 'TPOBITOCOR', 'DTCONINV', 'FONTES'])
    ...
    for col in np.array(['CODMUNNATU', 'CODMUNRES', 'CODMUNOCOR']):
        df[col].replace(['000000', '150475', '421265', '422000', '431454', '500627','990002', '990010', '990014', '999999'], '', inplace=True)
        ...
        df[col].replace(['530000', '530500', '530600', '530800', '530900', '531700', '539901', '539902', '539904', '539905', '539906', '539907',
                         '539914', '539916', '539918', '539919', '539920', '539921', '539924', '539925'], '530010', inplace=True)
    ...
    df['TPPOS'].replace('2', '0', inplace=True) # "2", representativo de "Não", é convertido para o objeto string "0" do domínio binário
    ...
    for col in np.array(['TIPOBITO', 'NATURAL', 'CODMUNNATU', 'RACACOR', 'ESTCIV', 'ESC', 'ESC2010', 'OCUP', 'CODMUNRES',
                         'LOCOCOR', 'CODESTAB', 'CODMUNOCOR', 'TPMORTEOCO', 'CAUSABAS', 'CIRCOBITO', 'FONTE', 'CAUSABAS_O',
                         'ATESTANTE', 'FONTEINV', 'ESCMAEAGR1', 'ESCFALAGR1', 'TPOBITOCOR', 'FONTES']):
        df[col].replace('', 'NA', inplace=True)

    # Substitui uma string vazia por None nas colunas de atributos especificadas
    for col in np.array(['CODINST', 'HORAOBITO', 'NUMSUS', 'SEXO', 'LINHAA', 'LINHAB', 'LINHAC', 'LINHAD', 'LINHAII', 'CRM', 'ATESTADO']):
        df[col].replace('', None, inplace=True)

    # Divisão da coluna "FONTES" em seis colunas conforme Dicionário de Dados da Tabela DOM ("M" de investigação materna)
    ...
    for col in np.array(['FONTENTREV', 'FONTEAMBUL', 'FONTEPRONT', 'FONTESVO', 'FONTEIML', 'FONTEPROF']):
        df[col] = df['FONTES'].apply(lambda x: 'NA' if x == 'NA' else x[0]) # O valor quando a condição "else" se verifica é "S" de "Sim"
        df[col].replace('X', '0', inplace=True) # Substitui a string "X" por "0" de "Não" tornando a coluna "col" com domínio "binário"
        df[col].replace('S', '1', inplace=True) # Substitui a string "X" por "1" de "Sim" tornando a coluna "col" com domínio "binário"
    ...
    for col in np.array(['DTOBITO', 'DTNASC', 'DTATESTADO', 'DTINVESTIG', 'DTCADASTRO', 'DTRECEBIM', 'DTCADINV', 'DTCONINV']):
        df[col] = df[col].apply(lambda x: datetime.strptime(x, '%d%m%Y').date() if x != '' else datetime(2099, 1, 1).date())
    ...
    for col in np.array(['DTOBITO', 'DTATESTADO', 'DTINVESTIG', 'DTCADASTRO', 'DTRECEBIM', 'DTCADINV', 'DTCONINV']):
        df[col] = df[col].apply(lambda x: x if datetime(2000, 12, 31).date() < x < datetime(2020, 12, 31).date() else datetime(2099, 1, 1).date())

    df['DTNASC'] = df['DTNASC'].apply(lambda x: x if datetime(1850, 12, 31).date() < x < datetime(2020, 12, 31).date() else datetime(2099, 1, 1).date())

    # Computa a diferença entre as datas de óbito e de nascimento em dias e a aloca como a coluna "IDADE" do objeto pandas DataFrame
    df['IDADE'] = df['DTOBITO'] - df['DTNASC']

    # Converte os valores da coluna IDADE de datetime.timedelta para string
    # Ainda na mesma linha, cria uma lista de dois objetos string de cada valor da coluna IDADE e aproveita apenas o primeiro objeto de cada lista
    df['IDADE'] = df['IDADE'].apply(lambda x: str(x).split(' day')[0])

    # Os valores em que a operação anterior forneceu a string "0:00:00" são substituídos pela string "0" (RN que viveram menos de um dia)
    df['IDADE'].replace('0:00:00', '0', inplace=True)

    # Converte os valores da coluna IDADE de string para float (em dias) atribuindo o float NaN para as string que começam com "-"
    ...
    for col in np.array(['ASSISTMED', 'EXAME', 'CIRURGIA', 'NECROPSIA', 'ACIDTRAB', 'TPPOS', 'STDOEPIDEM', 'STDONOVA',
                         'FONTENTREV', 'FONTEAMBUL', 'FONTEPRONT', 'FONTESVO', 'FONTEIML', 'FONTEPROF']):
    ...
    # Renomeia colunas
    df.rename(index=str, columns={'TIPOBITO': 'TIPOBITO_ID', 'NATURAL': 'NATURALE_ID', 'CODMUNNATU': 'CODMUNNATU_ID', 'RACACOR': 'RACACOR_ID',
                                  'ESTCIV': 'ESTCIV_ID', 'ESC': 'ESC_ID', 'ESC2010': 'ESC2010_ID', 'OCUP': 'OCUP_ID', 'CODMUNRES': 'CODMUNRES_ID',
                                  'LOCOCOR': 'LOCOCOR_ID', 'CODESTAB': 'CODESTAB_ID', 'CODMUNOCOR': 'CODMUNOCOR_ID', 'TPMORTEOCO': 'TPMORTEOCO_ID',
                                  'CAUSABAS': 'CAUSABAS_ID', 'CIRCOBITO': 'CIRCOBITO_ID', 'FONTE': 'FONTE_ID', 'CAUSABAS_O': 'CAUSABAS_O_ID',
                                  'ATESTANTE': 'ATESTANTE_ID', 'FONTEINV': 'FONTEINV_ID', 'ESCMAEAGR1': 'ESCMAEAGR1_ID', 'ESCFALAGR1': 'ESCFALAGR1_ID',
                                  'TPOBITOCOR': 'TPOBITOCOR_ID'}, inplace=True)
    print('Terminou de tratar o arquivo DO{}{} (shape final: {} x {}).'.format(state, year, df.shape[0], df.shape[1]))
    return df


# Função para adequar e formatar as colunas e valores da TCC TIPOBITO (arquivo TIPOBITO.cnv)
def get_TIPOBITO_treated():
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela DOBR
    df.loc[df.shape[0]] = ['NA', 'NOT AVAILABLE']
    return df


# Função para adequar e formatar as colunas e valores da TCC NAT1212 (arquivo NAT1212.cnv)
def get_NAT1212_treated(path):
    ...
    # Upload do arquivo "xlsx" que contém os NATURAL presentes nos arquivos DOXXxxxx (a partir do ano de 2001) e não presentes...
    # na TCC NAT1212. Ou seja, isso parece ser uma falha dos dados do Datasus
    dataframe = pd.read_excel(path + 'NATURAL_OUT_NAT1212_ANOS_1997_2017' + '.xlsx')
    # Converte a coluna "ID" do objeto "dataframe" de "int" para "string"
    dataframe['ID'] = dataframe['ID'].astype('str')
    # Adiciona zeros à esquerda nos valores (tipo string) da coluna "ID" do objeto "dataframe" até formar uma "string" de tamanho = 3
...
def get_TABOCUP_2TCC_treated(path):
    ...
    df.reset_index(drop=True, inplace=True)
    # Upload do arquivo "xlsx" que contém os OCUP presentes nos arquivos DOXXaaaa (dos anos de 1997 a 2017) e não presentes...
    # na Tabela TABOCUP e nas TCC CBO2002 e OCUPA. Ou seja, isso parece ser uma falha dos dados do Datasus
    dataframe = pd.read_excel(path + 'OCUP_OUT_TABOCUP_E_2TCC_ANOS_1997_2017' + '.xlsx')
    # Converte a coluna "ID" do objeto "dataframe" de "int" para "string"
    dataframe['ID'] = dataframe['ID'].astype('str')
...
def get_CNESDO18_3TCC_treated(path):
    ...
    # Reset o index devido ao sorting prévio
    df1.reset_index(drop=True, inplace=True)
    # Adiciona zeros à esquerda nos valores (tipo string) da coluna "CODESTAB" do objeto "df1" até formar uma "string" de tamanho = 7
    ...
    # Adiciona zeros à esquerda nos valores (tipo string) da coluna "CODESTAB" do objeto "df2" até formar uma "string" de tamanho = 7
    ...
    # Adiciona zeros à esquerda nos valores (tipo string) da coluna "CODESTAB" do objeto "df3" até formar uma "string" de tamanho = 7
    ...
    # Adiciona zeros à esquerda nos valores (tipo string) da coluna "CODESTAB" do objeto "df4" até formar uma "string" de tamanho = 7
    ...
    # Upload do arquivo "xlsx" que contém os CODESTAB presentes nos arquivos DOXXaaaa (dos anos de 1997 a 2017) e não presentes...
    # na tabela CNESDO18 e nas TCC ESTAB06, ESFERA18 e NAT_ORG. Ou seja, isso parece ser uma falha dos dados do Datasus
    dataframe = pd.read_excel(path + 'CODESTAB_OUT_CNESDO18_E_3TCC_ANOS_1997_2017' + '.xlsx')
    # Converte a coluna "CODESTAB" do objeto "dataframe" de "int" para "string"
    dataframe['CODESTAB'] = dataframe['CODESTAB'].astype('str')
    # Adiciona zeros à esquerda nos valores (tipo string) da coluna "CODESTAB" do objeto "dataframe" até formar uma "string" de tamanho = 7
...
# Função para adequar e formatar as colunas e valores da Tabela TPMORTEOCO ("constando" apenas do Dicionário de Dados do SIM)
def get_TPMORTEOCO_treated(path):
    # Conversão da Tabela TPMORTEOCO (em formato "xlsx") para um objeto pandas DataFrame
    df = pd.read_excel(path + 'TPMORTEOCO' + '.xlsx')
    # Renomeia a coluna SIGNIFICACAO
...
# Função para adequar e formatar as colunas e valores da Tabela CID10 (arquivo CID10.dbf) e de 21 TCC com nome CID10_XX (arquivos "cnv")
# sendo XX indo de 01 a 21, um para cada capítulo do CID 10.
def get_CID10_treated(path):
    ...
    # Reset o index devido ao sorting prévio e à eventual eliminação de duplicates
    df.reset_index(drop=True, inplace=True)
    # Upload do arquivo "xlsx" que contém os CAUSABAS OU CAUSABAS_O presentes nos arquivos DOXXaaaa (dos anos de 1997 a 2017) e não presentes...
    # na Tabela CID10 ou nas TCC CID10. Ou seja, isso parece ser uma falha dos dados do Datasus
    dataframe = pd.read_excel(path + 'CAUSABAS_OUT_CID10_ANOS_1997_2017' + '.xlsx')
...
# Função para adequar e formatar as colunas e valores da Tabela TPOBITOCOR ("constando" apenas do Dicionário de Dados do SIM)
def get_TPOBITOCOR_treated(path):
    # Conversão da Tabela TPOBITOCOR (em formato "xlsx") para um objeto pandas DataFrame
    df = pd.read_excel(path + 'TPOBITOCOR' + '.xlsx')
    # Renomeia a coluna SIGNIFICACAO
    df.rename(index=str, columns={'SIGNIFICACAO': 'EPOCA_MORTE'}, inplace=True)
    # Converte para string a coluna especificada
    df['ID'] = df['ID'].astype('str')
...
def get_TABUF_treated():
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" das tabelas CODMUNNATU, CODMUNRES, CODMUNOCOR
    df.loc[df.shape[0]] = ['NA', 'NOT AVAILABLE', '?']
    return df
