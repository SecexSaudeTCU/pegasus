...
# Função para ler como um objeto pandas DataFrame um arquivo de dados DNXXaaaa do SINASC e adequar e formatar suas colunas e valores
def get_DNXXaaaa_treated(state, year):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e o salva no formato "parquet"
    dataframe = download_DNXXaaaa(state, year)
    print('O número de linhas do arquivo DN{}{} é {}.'.format(state, year, dataframe.shape[0]))
    ...
    lista_columns = np.array(['NUMERODN', 'CODINST', 'CODESTAB', 'CODMUNNASC', 'LOCNASC', 'IDADEMAE', 'ESTCIVMAE', 'ESCMAE',
                              'CODOCUPMAE', 'QTDFILVIVO', 'QTDFILMORT', 'CODMUNRES', 'GESTACAO', 'GRAVIDEZ', 'PARTO', 'CONSULTAS',
                              'DTNASC', 'HORANASC', 'SEXO', 'APGAR1', 'APGAR5', 'RACACOR', 'PESO', 'IDANOMAL', 'DTCADASTRO',
                              'CODANOMAL', 'DTRECEBIM', 'DIFDATA', 'NATURALMAE', 'CODMUNNATU', 'ESCMAE2010', 'DTNASCMAE',
                              'RACACORMAE', 'QTDGESTANT', 'QTDPARTNOR', 'QTDPARTCES', 'IDADEPAI', 'DTULTMENST', 'SEMAGESTAC',
                              'TPMETESTIM', 'CONSPRENAT', 'MESPRENAT', 'TPAPRESENT', 'STTRABPART', 'STCESPARTO', 'TPNASCASSI',
    ...
    for col in np.array(['CODMUNNASC', 'CODMUNRES', 'CODMUNNATU']):
        ...
        df[col].replace(['000000', '150475', '241005', '282580', '279982', '282586', '292586', '315205',
                         '421265', '422000', '431454', '500627', '990010', '990014', '999999'], '', inplace=True)
        ...
        df[col].replace(['530000', '530100', '530200', '530300', '530400', '530500', '530600', '530700',
    ...
    df['IDANOMAL'].replace('2', '0', inplace=True) # "2", representativo de "Não", é convertido para o objeto string "0" do domínio binário
    ...
    for col in np.array(['CODESTAB', 'CODMUNNASC', 'LOCNASC', 'ESTCIVMAE', 'ESCMAE', 'CODOCUPMAE', 'CODMUNRES', 'GESTACAO', 'GRAVIDEZ',
                         'PARTO', 'CONSULTAS', 'RACACOR', 'CODANOMAL', 'NATURALMAE', 'CODMUNNATU', 'ESCMAE2010', 'RACACORMAE', 'TPMETESTIM',
                         'TPAPRESENT', 'STTRABPART', 'STCESPARTO', 'TPNASCASSI', 'TPFUNCRESP', 'ESCMAEAGR1', 'CODPAISRES', 'TPROBSON']):
        df[col].replace('', 'NA', inplace=True)

    # Substitui uma string vazia por None nas colunas de atributos especificadas
    for col in np.array(['CODINST', 'HORANASC', 'SEXO', 'MESPRENAT', 'TPDOCRESP', 'KOTELCHUCK']):
    ...
    for col in np.array(['DTNASC', 'DTCADASTRO', 'DTRECEBIM', 'DTNASCMAE', 'DTULTMENST', 'DTDECLARAC']):
    ...
    for col in np.array(['DTNASC', 'DTCADASTRO', 'DTRECEBIM', 'DTDECLARAC']):
        df[col] = df[col].apply(lambda x: x if datetime(2000, 12, 31).date() < x < datetime(2025, 12, 31).date() else datetime(2099, 1, 1).date())
    df['DTNASCMAE'] = df['DTNASCMAE'].apply(lambda x: x if datetime(1900, 12, 31).date() < x < datetime(2025, 12, 31).date() else datetime(2099, 1, 1).date())
    df['DTULTMENST'] = df['DTULTMENST'].apply(lambda x: x if datetime(1990, 12, 31).date() < x < datetime(2025, 12, 31).date() else datetime(2099, 1, 1).date())
    ...
    for col in np.array(['IDADEMAE', 'QTDFILVIVO', 'QTDFILMORT', 'APGAR1', 'APGAR5', 'PESO', 'DIFDATA', 'QTDGESTANT', 'QTDPARTNOR', 'QTDPARTCES',
                         'IDADEPAI', 'SEMAGESTAC', 'CONSPRENAT']):
    ...
    # Renomeia colunas
    df.rename(index=str, columns={'CODESTAB': 'CODESTAB_ID', 'CODMUNNASC': 'CODMUNNASC_ID', 'LOCNASC': 'LOCNASC_ID', 'ESTCIVMAE': 'ESTCIVMAE_ID',
                                  'ESCMAE': 'ESCMAE_ID', 'CODOCUPMAE': 'CODOCUPMAE_ID', 'CODMUNRES': 'CODMUNRES_ID', 'GESTACAO': 'GESTACAO_ID',
                                  'GRAVIDEZ': 'GRAVIDEZ_ID', 'PARTO': 'PARTO_ID', 'CONSULTAS': 'CONSULTAS_ID', 'RACACOR': 'RACACOR_ID',
                                  'CODANOMAL': 'CODANOMAL_ID', 'NATURALMAE': 'NATURALMAE_ID', 'CODMUNNATU': 'CODMUNNATU_ID', 'ESCMAE2010': 'ESCMAE2010_ID',
                                  'RACACORMAE': 'RACACORMAE_ID', 'TPMETESTIM': 'TPMETESTIM_ID', 'TPAPRESENT': 'TPAPRESENT_ID', 'STTRABPART': 'STTRABPART_ID',
                                  'STCESPARTO': 'STCESPARTO_ID', 'TPNASCASSI': 'TPNASCASSI_ID', 'TPFUNCRESP':'TPFUNCRESP_ID', 'ESCMAEAGR1': 'ESCMAEAGR1_ID',
                                  'CODPAISRES': 'CODPAISRES_ID', 'TPROBSON': 'TPROBSON_ID'}, inplace=True)

    print('Terminou de tratar o arquivo DN{}{} (shape final: {} x {}).'.format(state, year, df.shape[0], df.shape[1]))
...
def get_CNESDN_treated(path):
    ...
    # Upload do arquivo "xlsx" que contém os CODESTAB presentes nos arquivos DNXXaaaa (dos anos de 1997 a 2017) e não presentes...
    # na tabela CNESDN18 e na TCC CNESDN07. Ou seja, isso parece ser uma falha dos dados do Datasus
    dataframe = pd.read_excel(path + 'CODESTAB_OUT_CNESDN_07_E_18_ANOS_1997_2017' + '.xlsx')
    ...
    # Adiciona as colunas DESCESTAB, ESFERA e REGIME e respectivos valores ao objeto "dataframe" para torná-lo com as mesmas...
    # colunas do "df6"
...
# Função para adequar e formatar as colunas e valores da Tabela CADMUN (sendo originalmente o arquivo CADMUN.dbf)
def get_CADMUN_treated():
    # Conversão da Tabela CADMUN (em formato "dbf") para um objeto pandas DataFrame
    file_name = 'CADMUN'
    ...
    df.reset_index(drop=True, inplace=True)
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela DNBR
...
def get_TABOCUP_2TCC_treated(path):
    ...
    dataframe = pd.read_excel(path + 'CODOCUPMAE_OUT_TABOCUP_E_2TCC_ANOS_1997_2017' + '.xlsx')
...
# Utiliza-se esses 21 CID10_XX.cnv do SIM ao invés do CID1017.cnv do SINASC pois aquele é completo e este contém apenas o capítulo 17
def get_CID10_treated():
    ...
    # Reset o index devido ao sorting prévio e à eventual eliminação de duplicates
    df.reset_index(drop=True, inplace=True)
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da coluna "CODANOMAL_ID" da tabela DNBR
    df.loc[df.shape[0]] = ['NA', 'NOT AVAILABLE']
    return df


# Função para adequar e formatar as colunas e valores da TCC NAT1212 (arquivo NAT1212.cnv)
# Utiliza-se o NAT1212.cnv (pertencente do SIM) por não constar nenhuma referência a um "cnv" no Dicionário de Dados do SINASC
def get_NAT1212_treated():
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar célula de string vazia da coluna "NATURALMAE_ID" da tabela DNBR
    df.loc[df.shape[0]] = ['NA', 'NOT AVAILABLE']
    return df


# Função para adequar e formatar as colunas e valores da TCC ESC2010 (arquivo ESC2010.cnv)
def get_ESC2010_treated():
...
# Função para adequar e formatar as colunas e valores da Tabela TPAPRESENT ("constando" apenas do Dicionário de Dados do SINASC)
def get_TPAPRESENT_treated(path):
...
# Função para adequar e formatar as colunas e valores da Tabela STTRABPART ("constando" apenas do Dicionário de Dados do SINASC)
def get_STTRABPART_treated(path):
...
# Função para adequar e formatar as colunas e valores da Tabela TPFUNCRESP ("constando" apenas do Dicionário de Dados do SINASC)
def get_TPFUNCRESP_treated(path):
...
def get_TABUF_treated():
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" das tabelas CODMUNNASC e CODMUNRES
    df.loc[df.shape[0]] = ['NA', 'NOT AVAILABLE', '?']
    return df
