...
"""
Script de tratamento de dados do CNES_EQ (Equipamentos) para atender ao framework do SGBD PostgreSQL.
Válido para os arquivos de dados EQXXaamm (EQ = Equipamentos; XX = Estado; aa = Ano; mm = Mês)...
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados EQXXaamm do CNES e adequar e formatar suas colunas e valores
def get_EQXXaamm_treated(state, year, month):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e o salva no formato "parquet"
    dataframe = download_CNESXXaamm('EQ' , state, year, month)
    print('O número de linhas do arquivo EQ{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))

    # Colunas definidas como necessárias no objeto pandas DataFrame que incrementará a tabela EQBR da base de dados
    ...
    print('Terminou de tratar o arquivo EQ{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
def get_CADGERBR_treated(path):
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela EQBR
...
# Função para adequar e formatar as colunas e valores da Tabela TP_EQUIPAM (arquivo TP_EQUIPAM.dbf)
def get_TP_EQUIPAM_treated():
...
# Função para adequar e formatar as colunas e valores da TCC Equip_Tp (arquivo Equip_Tp.cnv)
def get_Equip_Tp_treated():
