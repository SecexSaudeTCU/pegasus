...
"""
Script de tratamento de dados do CNES_LT (Leitos) para atender ao framework do SGBD PostgreSQL.
Válido para os arquivos de dados LTXXaamm (LT = Leitos; XX = Estado; aa = Ano; mm = Mês)...
...
# Função para ler como um objeto pandas DataFrame um arquivo de dados LTXXaamm do CNES e adequar e formatar suas colunas e valores
def get_LTXXaamm_treated(state, year, month):
    # Lê o arquivo "dbc" como um objeto pandas DataFrame e o salva no formato "parquet"
    dataframe = download_CNESXXaamm('LT' , state, year, month)
    print('O número de linhas do arquivo LT{}{}{} é {}.'.format(state, year, month, dataframe.shape[0]))

    # Colunas definidas como necessárias no objeto pandas DataFrame que incrementará a tabela LTBR da base de dados
    ...
    print('Terminou de tratar o arquivo LT{}{}{} (shape final: {} x {}).'.format(state, year, month, df.shape[0], df.shape[1]))
...
def get_CADGERBR_treated(path):
    ...
    # Inserção da primary key "NA" na tabela de que trata esta função para retratar "missing value" da tabela LTBR
...
# Função para adequar e formatar as colunas e valores da TCC tip1leit (arquivo tip1leit.cnv)
def get_tip1leit_treated():
...
# Função para adequar e formatar as colunas e valores da TCC Esp_leit (arquivo Esp_leit.cnv)
def get_Esp_leit_treated():
