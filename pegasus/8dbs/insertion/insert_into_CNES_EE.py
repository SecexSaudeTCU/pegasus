...
from .data_wrangling.prepare_CNES_EE import *
...
def insert_most_CNES_EE_tables_pandas(path, device, child_db):
...
def insert_main_table_e_file_info_pandas(file_name, directory, date_ftp, device, child_db, parent_db):
    ...
    # Tratamento de dados principais do CNES_EE
    state = file_name[2:4]
    year = file_name[4:6]
    month = file_name[6:8]
    main_table = 'eebr'
    ...
    print(f'\nIniciando a lida com o arquivo EE{state}{year}{month}...')
    # Chama a função "get_EEXXaamm_treated" do módulo "prepare_CNES_EE" do package "data_wrangling"
    df = get_EEXXaamm_treated(state, year, month)
    # Inserção das colunas UF_EE, ANO_EE e MES_EE no objeto pandas DataFrame "df"
    df.insert(1, 'UF_EE', [state]*df.shape[0])
    df.insert(2, 'ANO_EE', [int('20' + year)]*df.shape[0])
    df.insert(3, 'MES_EE', [month]*df.shape[0])
    ...
    print(f'Terminou de inserir os dados do arquivo EE{state}{year}{month} na tabela {main_table} do banco de dados {child_db}.')
    ...
    print(f'Terminou de inserir os metadados do arquivo EE{state}{year}{month} na tabela arquivos do banco de dados {child_db}.')
    end = time.time()
