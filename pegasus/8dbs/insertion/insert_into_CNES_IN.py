...
from .data_wrangling.prepare_CNES_IN import *
...
def insert_most_CNES_IN_tables_pandas(path, device, child_db):
...
def insert_main_table_e_file_info_pandas(file_name, directory, date_ftp, device, child_db, parent_db):
    ...
    # Tratamento de dados principais do CNES_IN
    state = file_name[2:4]
    year = file_name[4:6]
    month = file_name[6:8]
    main_table = 'inbr'
    ...
    print(f'\nIniciando a lida com o arquivo IN{state}{year}{month}...')
    # Chama a função "get_INXXaamm_treated" do módulo "prepare_CNES_IN" do package "data_wrangling"
    df = get_INXXaamm_treated(state, year, month)
    # Inserção das colunas UF_IN, ANO_IN e MES_IN no objeto pandas DataFrame "df"
    df.insert(1, 'UF_IN', [state]*df.shape[0])
    df.insert(2, 'ANO_IN', [int('20' + year)]*df.shape[0])
    df.insert(3, 'MES_IN', [month]*df.shape[0])
    ...
    print(f'Terminou de inserir os dados do arquivo IN{state}{year}{month} na tabela {main_table} do banco de dados {child_db}.')
    ...
    print(f'Terminou de inserir os metadados do arquivo IN{state}{year}{month} na tabela arquivos do banco de dados {child_db}.')
    end = time.time()
