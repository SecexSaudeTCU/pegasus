...
from .data_wrangling.prepare_CNES_EF import *
...
def insert_most_CNES_EF_tables_pandas(path, device, child_db):
...
def insert_main_table_e_file_info_pandas(file_name, directory, date_ftp, device, child_db, parent_db):
    ...
    # Tratamento de dados principais do CNES_EF
    state = file_name[2:4]
    year = file_name[4:6]
    month = file_name[6:8]
    main_table = 'efbr'
    ...
    print(f'\nIniciando a lida com o arquivo EF{state}{year}{month}...')
    # Chama a função "get_EFXXaamm_treated" do módulo "prepare_CNES_EF" do package "data_wrangling"
    df = get_EFXXaamm_treated(state, year, month)
    # Inserção das colunas UF_EF, ANO_EF e MES_EF no objeto pandas DataFrame "df"
    df.insert(1, 'UF_EF', [state]*df.shape[0])
    df.insert(2, 'ANO_EF', [int('20' + year)]*df.shape[0])
    df.insert(3, 'MES_EF', [month]*df.shape[0])
    ...
    print(f'Terminou de inserir os dados do arquivo EF{state}{year}{month} na tabela {main_table} do banco de dados {child_db}.')
