...
def load_all(db_name, db_user, db_password, first_year, last_year):
    ...
    if decision == 'q':
        ...
        if datasus_db == 'sia':
            ...
            print('\nUse o módulo "load_sub-db.py" (e apenas foi implementado o ETL da sub-base de dados SINAN_DENG)!')
            quit()
        else:
            print('\nTo its goal...\n')
       ...
       # Importação da função "create_tables" de criação do schema do banco de dados "datasus_db" existente no respectivo
        ...
        if ((dict_tabelas_e_counts_pg == dict()) or (count > 0)):
            ...
            if datasus_db == 'cnes':
                ...
                os.remove('OBITOS_CID10_TAB.ZIP') # e do arquivo CNESDO22.dbf
            elif datasus_db == 'sinasc':
                os.remove('NASC_NOV_TAB.zip')
                os.remove('OBITOS_CID10_TAB.ZIP') # e do arquivo CNESDN22.dbf
